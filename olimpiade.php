...
<body>
  <?php
    ...
    ?>
    <?php
    $query = mysqli_query($con, "SELECT * FROM info_lomba where id_info = 2 ");
...
<main id="main">
	...
	<div class="container">
		<h2 class="section-title2"><span>OLIMPIADE</span></h2>
		...
		<!-- OPSI 1 -->
		<h2 class="section-title3"><span class="glyphicon glyphicon-cog" style="font-size:20px; color:#eac338"></span> Peraturan Perlombaan Olimpiade</h2>
		
		<h2 class="section-title4"><b>Multiple Choice</b></h2>
		
			<table>
            <?php 
            $query = mysqli_query($con, "SELECT p_lomba FROM peraturan_lomba where id_info = 20 order by id_peraturan");
            while($p = mysqli_fetch_array($query)){
                ?>
                <tr><th class="akutes"><span class="glyphicon glyphicon-pencil" style="font-size:12px; color:#292929"></span></th><th><?php print $p['p_lomba'];?></th></tr>
                <?php } ?>
         </table>
		
		<hr class="hr-style3"></hr>
			<h2 class="section-title4"><b>Kasus</b></h2>
		<table>
            <?php 
            $query = mysqli_query($con, "SELECT p_lomba FROM peraturan_lomba where id_info = 2 order by id_peraturan");
            while($p = mysqli_fetch_array($query)){
                ?>
