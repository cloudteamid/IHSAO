...
<main id="main">
	...
	<div class="container">
		<h2 class="section-title2"><span>Daftar Peserta IHSAO</span></h2>
		<div class="row section topspace">
			<div class="col-md-12">
				<p class="lead text-center">Kamu dapat melihat apakah sudah terdaftar di acara IHSAO 2019 (khusus pendaftar online).<br>
				Jika belum <b>upload Bukti Pembayaran</b>, silahkan upload <a href="upload.php"> disini!</a></p>
			</div>
		</div> 
		<hr class="hr-style1"><br>
		<table class="table-listpeserta">
		    <form action="list-peserta.php" method="get">
		    <tr>
		        <th class="col-md-12"><p class="lead text-center">Masukkan ID Peserta
		            <input class="form-listpeserta" type ="Text" name="searchname">
		            <button class="tombolcek-listpeserta" data-text="Daftar">Cek</button></p></th>
		    </tr>
		    </form>
		    <?php 
            if($_GET['searchname']){
	        $cari = $_GET['searchname'];
            }
            ?>
		    </table>
		<table class="informasi-isi">
		        <tr><th class="listpeserta-table" style="width : 15%"><b>No</b></th><th class="listpeserta-table"><b>ID Team</b></th><th class="listpeserta-table"><b>Status</b></th></tr>
		    	<?php 
	            if($_GET['searchname']){
	        	$cari = $_GET['searchname'];
	        	$a = 0;
	        	$data = mysqli_query($con ,"SELECT * FROM pendaftaran where kode like '%".$cari."%'");				
            	}else{
	        	   $data = mysqli_query($con ,"SELECT kode,status FROM pendaftaran");		
	            }
	            while($d = mysqli_fetch_array($data)){$a++
	            ?>
	       	    <tr>
	       	<th><center><?php print $a; ?></center></th>
		    <th><center><?php print $d['kode']; ?></center></th>
		    <th><center><?php print $d['status']; ?></center></th>
        	</tr>
    	<?php } ?>
		</table>
